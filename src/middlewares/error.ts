...
static handle( error:Error,req: t , res :Response , next : NextFunction){
    console.log("heloo mmmmm", req.originalUrl) 
    console.log(process.env.NODE_ENV);
    dotenv.config({
        path: path.join(__dirname,"..",".env")
    });
    if(req.originalUrl.startsWith("/product"))return res.status(500).render("componants/error.pug" , {title:"Internal Server Error", "status":500} );
